...
function normalizeStatusFromUazapi(
  status: string,
  connected: boolean,
  loggedIn: boolean,
): ConnectionStatus {
...
export function createWhatsappRouter(
  ...
  router.post("/instance", async (c) => {
      ...
      return c.json(
        {
          error: {
            code: "FORBIDDEN",
            message: "Apenas owner pode criar instância",
          },
        },
        403,
      );
    ...
    const bodyResult = CreateInstanceBodySchema.safeParse(
    ...
    if (existingError) {
      return c.json(
        { error: { code: "INTERNAL", message: "Erro ao ler sessão" } },
        500,
      );
    }
    ...
    try {
      created = await uazapiDeps.createInstance({
        name,
        adminField01: tenantId,
      });
    } catch (err) {
      if (err instanceof UazapiUnauthorizedError) {
        return c.json(
          {
            error: {
              code: "UAZAPI_MISCONFIGURED",
              message: "Servidor WhatsApp não configurado. Contacte o suporte.",
            },
          },
          503,
        );
    ...
    if (upsertError) {
      return c.json(
        { error: { code: "INTERNAL", message: "Erro ao persistir instância" } },
        500,
      );
    }

    return c.json(
      { instanceId: created.instanceId, name, status: "disconnected" },
      201,
    );
  });
  ...
  router.delete("/instance", async (c) => {
      ...
      return c.json(
        {
          error: {
            code: "FORBIDDEN",
            message: "Apenas owner pode remover instância",
          },
        },
        403,
      );
    ...
    if (deleteError) {
      return c.json(
        {
          error: { code: "INTERNAL", message: "Erro ao remover sessão local" },
        },
        500,
      );
    }
  ...
  router.post("/instance/connect", async (c) => {
      ...
      return c.json(
        {
          error: {
            code: "FORBIDDEN",
            message: "Apenas owner pode iniciar conexão",
          },
        },
        403,
      );
    ...
    const { data: sessionData, error: sessionError } = await db
      .from("whatsapp_sessions")
      .select(
        "uazapi_instance_id, uazapi_instance_token, uazapi_webhook_secret",
      )
      .eq("tenant_id", tenantId)
      .maybeSingle();
    ...
    const webhookSecret =
      typeof session.uazapi_webhook_secret === "string" &&
      session.uazapi_webhook_secret.length > 0
    ...
    try {
      ...
      if (updateError) {
        return c.json(
          { error: { code: "INTERNAL", message: "Erro ao atualizar sessão" } },
          500,
        );
      }
        ...
        return c.json(
          {
            error: {
              ...
              message:
                "Servidor WhatsApp sobrecarregado. Aguarde alguns segundos e tente novamente.",
            },
          },
          503,
        );
      }
      if (error instanceof UazapiUnauthorizedError) {
        return c.json(
          {
            error: {
              code: "UAZAPI_MISCONFIGURED",
              message:
                "Token de instância inválido. Exclua e recrie a instância.",
            },
          },
          503,
        );
      ...
      if (error instanceof UazapiNotFoundError) {
        return c.json(
          {
            error: {
              code: "INSTANCE_NOT_FOUND_ON_PROVIDER",
              message:
                "Instância não encontrada no servidor WhatsApp. Exclua e recrie a instância.",
            },
          },
          409,
        );
  ...
  router.get("/instance/status", async (c) => {
    ...
    const { data: sessionData, error } = await db
      .from("whatsapp_sessions")
      .select(
        "status, instance_name, phone_number, last_heartbeat_at, last_error, qr_expires_at, uazapi_instance_token",
      )
      .eq("tenant_id", tenantId)
      .maybeSingle();
    ...
    if ((status === "qr_pending" || status === "connecting") && token) {
      ...
      const update: SessionRow = {
        ...
        instance_name:
          ...
          (typeof session.instance_name === "string"
